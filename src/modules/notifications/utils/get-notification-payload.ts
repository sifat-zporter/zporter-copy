...
export const getNotificationPayload = (
  ...
  switch (type) {
    ...
    case NotificationType.TAG_FEED:
      return `#${username} tagged you in ${content === GenderTypes.Male ? 'his' : 'her'
        } posts.`;
    ...
    case NotificationType.VERIFY_TEST_RECORD:
      return `Can you verify this result - Yes / No.`;

    case NotificationType.CONFIRMED_TEST_RECORD:
      return `Your test result has been verified.`;

    case NotificationType.REFUSED_TEST_RECORD:
      return `Your test result has been refused. Please check it again!`;

    case NotificationType.VERIFY_TEST_RECORD_UPDATE:
      return `Can you verify this updated result - Yes / No.`;

    case NotificationType.ADDED_TEST_RECORD_BY_COACH:
      return `${username} has added your Test result. ${content}.`;

    case NotificationType.SEND_TEST_RECORD_TO_COACH:
      return `You has added Test result for ${username}. ${content}.`;

    case NotificationType.EVENT_CANCELLED:
      return `The event has been cancelled.`;

    case NotificationType.EVENT_INVITATION:
      return `Will you participate?`;
